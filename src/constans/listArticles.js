export const LIST_ARTICLES = [
    {
        title: "Tips Memilih Jasa Sumur Bor yang Tepat",
        slug: "tips-memilih-jasa-sumur-bor-yang-tepat",
        image: "/article/article-sumur-bor.webp",
        content: `Memilih jasa sumur bor bukan hal yang bisa dilakukan sembarangan. Kualitas pengeboran menentukan seberapa lancar dan bersih air yang kamu dapatkan. 
Berikut beberapa tips sebelum menentukan pilihan:
1. Pilih penyedia jasa berpengalaman yang memahami kondisi tanah di wilayahmu.
2. Pastikan dilakukan survei lokasi untuk menentukan kedalaman air yang tepat.
3. Gunakan jasa dengan peralatan modern dan aman.
4. Perhatikan garansi hasil dan layanan purna jual.

💧 Dengan penyedia jasa profesional seperti *Mata Air Nusantara*, kamu bisa mendapatkan air bersih dan sumur tahan lama tanpa khawatir masalah di kemudian hari.`,
        keywords: ["sumur bor", "jasa sumur bor", "air bersih", "mata air nusantara"]
    },
    {
        title: "Cara Merawat Pompa Air Agar Awet dan Tidak Mudah Rusak",
        slug: "cara-merawat-pompa-air-agar-awet-dan-tidak-mudah-rusak",
        image: "/article/article-pompa-air.webp",
        content: `Pompa air adalah bagian vital dari sistem air rumah tangga. Agar tetap awet, lakukan perawatan rutin:
1. Gunakan pompa sesuai kapasitasnya.
2. Periksa tekanan air secara berkala.
3. Bersihkan saringan (filter) dari kotoran dan endapan.
4. Matikan pompa saat tidak digunakan dalam waktu lama.
5. Gunakan teknisi profesional untuk servis rutin.

🧰 Perawatan ringan dan rutin bisa memperpanjang usia pompa serta menjaga aliran air tetap lancar setiap hari.`,
        keywords: ["pompa air", "perawatan pompa", "jasa sumur bor", "servis pompa air"]
    },
    {
        title: "Ciri-Ciri Air Sumur yang Layak Konsumsi",
        slug: "ciri-ciri-air-sumur-yang-layak-konsumsi",
        image: "/article/article-air-bersih.webp",
        content: `Tidak semua air sumur aman diminum. Berikut tanda-tanda air yang baik:
1. Jernih dan tidak berbau logam atau karat.
2. Tidak meninggalkan endapan di dasar wadah.
3. Rasa air netral — tidak pahit atau asam.
4. Hasil uji laboratorium menunjukkan bebas bakteri dan bahan kimia berbahaya.

🌿 Jika air mulai berubah warna atau bau, segera lakukan pemeriksaan atau gunakan jasa pembersihan sumur profesional.`,
        keywords: ["air sumur", "air bersih", "uji kualitas air", "mata air nusantara"]
    }
]